...
export const SwipeCard = React.forwardRef<SwipeCardPublicMethods, SwipeCardProps>(({ content, leftAction, rightAction, onSwipe, reward }, ref) => {
    ...
    return (
        ...
        >
            ...
            >
                {isExpanded ? <MinimizeIcon fill="currentColor" /> : <MaximizeIcon fill="currentColor" />}
            </button>

            <div
                className={`absolute inset-0 bg-black border-2 border-red-500 rounded-2xl transition-all duration-200 ${isExpanded ? 'cursor-zoom-out' : 'cursor-grab'
                    }`}
            ...
            <div className="absolute bottom-20 left-0 right-0 px-4">
                <div className="flex justify-center items-center space-x-4">
                    ...
                    {/* Bet Amount Display */}
                    <span className="text-yellow-500 font-bold text-6xl shadow-md">{betAmount}</span>
                ...
                <div className="mt-2 bg-gray-700 rounded-full h-2 relative">
                    ...
                    />
                    <span className="absolute left-0 top-full mt-1 text-white text-sm">0</span>
                    <span className="absolute right-0 top-full mt-1 text-white text-sm">{reward.points}</span>
                </div>
            </div>
            ...
            <div className="absolute bottom-4 left-0 right-0 px-4 flex justify-between items-center">
                {/* Left Action Button */}
                <button
                    onClick={() => onSwipe('left', betAmount)}
                ...
                >
                    <LeftArrowIcon fill="currentColor" className="absolute left-3 top-1/2 transform -translate-y-1/2 w-6 h-6" />
                    ...
                    <span className="absolute top-0 left-0 w-full h-full bg-white transform scale-0 rounded-full pointer-events-none duration-500 ease-out" style={{ animation: 'pulse-glow 1s ease-out infinite' }}></span>
                </button>

                {/* Right Action Button */}
                <button
                    onClick={() => onSwipe('right', betAmount)}
                ...
                >
                    ...
                    <RightArrowIcon fill="currentColor" className="absolute right-3 top-1/2 transform -translate-y-1/2 w-6 h-6" />
                    <span className="absolute top-0 left-0 w-full h-full rounded-full pointer-events-none opacity-0 hover:opacity-10 bg-white"></span>
                    <span className="absolute top-0 left-0 w-full h-full bg-white transform scale-0 rounded-full pointer-events-none duration-500 ease-out" style={{ animation: 'pulse-glow 1s ease-out infinite' }}></span>
                </button>
            </div>
...
const MaximizeIcon = ({ fill }: { fill: string }) => (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24">
        ...
        <path stroke={fill} strokeWidth="2" d="M7 7l-4 4m16-4l-4 4m4 8l-4-4m-16 4l4-4" />
    </svg>
);
...
const LeftArrowIcon = ({ fill, className }: { fill: string; className?: string }) => (
...
const RightArrowIcon = ({ fill, className }: { fill: string; className?: string }) => (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill={fill} className={className}>
        <path fillRule="evenodd" d="M12 2.25c5.385 0 9.75 4.365 9.75 9.75s-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12 6.615 2.25 12 2.25zm2.78 9.22a.75.75 0 00-1.06 1.06l3 3a.75.75 0 001.06-1.06l-1.72-1.72H8.25a.75.75 0 000-1.5h5.69l-1.72-1.72a.75.75 0 101.06-1.06l3 3z" clipRule="evenodd" />
    </svg>
);
